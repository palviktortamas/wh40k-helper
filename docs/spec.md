...
- **CORS reality (verified 2026-09-13):** `raw.githubusercontent.com` (BSData + MFM mirror) sends `Access-Control-Allow-Origin: *` → fetchable directly from the browser. **Wahapedia sends no CORS headers** → its CSVs and the mission-deck page cannot be fetched by the app directly. Therefore:
  ...
  - Do **not** mirror Wahapedia files into the public repo (see below), even via a scheduled Action. *(v0.8, owner's decision 2026-09-13: the deployed build may carry two Wahapedia files fetched at build time and never committed — the mission deck page (§6.1) and the stratagem export, which Play Mode's stratagem list needs (§6.3).)*
