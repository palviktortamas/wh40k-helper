/**
 * Exercises the constraint evaluator against every real catalogue the fixtures
 * offer — a second faction is what catches the places one codex's shape was
 * mistaken for the rules. Skipped unless WH40K_FIXTURES is set (see
...
describe.skipIf(!available)('constraint evaluator on real data', () => {
  describe.each(factions)('$name', (faction: FixtureFaction) => {
    const loadGraph = (): CatalogueGraph =>
      buildGraph(loadGameSystem(), loadCatalogueOf(faction), loadLibrariesOf(faction))
